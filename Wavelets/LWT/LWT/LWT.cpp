...
const int LOG_DATALENGTH(5);  // Must be equal to log(2,DATALENGTH),  logarithm of datalength
...
void LWT(WTfloat * x,  int wt_steps) 
{
	...
	for (int j=0; j<wt_steps; j++)		//computer the LWT stages	
	{
		WTfloat * y = x+offset;
		{
			d[0] = y[0]-y[1];	// front boundary treatment equivalent to inserting a leading zero
			for(int i=1; i<lens; i++)
			{
				int ii=i<<1;
				// dual lifting
				d[i] = y[ii] - (y[ii+1] + y[ii-1])/2;
				// primal lifting
				s[i-1] = y[ii-1] + (d[i-1] + d[i])/4;
			}
			s[lens-1] = y[2*lens-1] + d[lens-1]/2;	// end boundary treatment equivalent to inserting a trailing zero
		}
	...
	{
		{
			...
			s[0]= d1[0] + s[1];	// front boundary treatment
			s[offset-1]= s1[lenwt2-1]-d1[lenwt2-1]/ 2;	// back boundary treatment: more symmetrical treatment is achieved by the factor of 2

			for (int i=1; i<lenwt2; i++)
			{
				int ii=i<<1;
				s[ii] = d1[i] + (s[ii-1]+s[ii+1])/2;
			}
		}
...
void main(){
	...
	if (DEBUG) printf("We are going to transform the following data:\n");
	for(i=0;i<DATALENGTH;i++){
		aaa[i]=sin((double)(i+3)*(i+2));  
		//		aaa[i]=rand();
		bbb[i]=aaa[i];
		if (DEBUG) 
		{		
